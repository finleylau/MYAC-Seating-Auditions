...
def readRoster(roster, computer_user):
	...
	for row in reader:
		if string.capwords(row['Orchestra']) == 'Concert':
			...
			writer.writerow(row)
			makeStudentFolder(row,computer_user)
...
def makeStudentFolder(student,computer_user):

	# creates path to folder
	dropbox_location = 'C:\\Users\\' + computer_user + '\\Dropbox\\'
	folder_name = 'Seating Audition Folders with Names'
	folder_path = '\\' + string.capwords(student['Orchestra']) + '\\' + string.capwords(student['Instrument']) + '\\' + string.capwords(student['Last Name']) + ', ' + string.capwords(student['First Name'])

	# creates directories
	path = dropbox_location + folder_name + folder_path
	if not os.path.exists(path):
		os.makedirs(path)

# takes in a list of codes as a input and creates folders for each code
def makeInstrumentFolders(codes, computer_user):
	...
	for code in codes:
		code_path = makeCodeFilePath(code, concert_instrument_codes, symphony_instrument_codes, computer_user)
...
def makeCodeFilePath(code, concert_codes, symphony_codes, computer_user):
